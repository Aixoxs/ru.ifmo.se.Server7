...
public class Collection implements Serializable {
    ...
    public String update(MusicBand mb, User user) {
        ...
        if (result.equals("true")) {
            ...
            if (musicBandDao.update(mb)) {
                ...
                result = "Объект успешно добавлен в коллекцию";
            } else {
    ...
    public String show() {
        ...
        if(musicBands.size() == 0){
            result.append("Элемент не найден");
        }else {
            musicBands.stream().filter((o)->o.getFrontMan().getNationality()!=null).sorted(Comparator.comparing(x -> x.getFrontMan().getNationality())).forEach((o) -> result.append(o.toString()).append("\n"));
            musicBands.stream().filter((o)->o.getFrontMan().getNationality()==null).forEach((o) -> result.append(o.toString()).append("\n"));
        }
